pipeline {
    ...
    environment {
        ...
        IMAGE_NAME = 'fintrack-telegram-service'
        CONTAINER_NAME = 'fintrack-telegram-service-container'
    ...
    stages {
        ...
        //                     -Dsonar.projectKey=fintrack-telegram-service \
        //                     -Dsonar.sources=src \
        ...
        stage('Trigger ManifestUpdate') {
            steps {
                script {
                    try {
                        build job: 'telegram-service-pipeline-2', parameters: [string(name: 'DOCKERTAG', value: env.BUILD_NUMBER)]
                    } catch (Exception e) {
                        currentBuild.result = 'FAILURE'
